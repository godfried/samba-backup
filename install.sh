...
BACKUP_NAME="samba_backup"
BACKUP_ENVRC="${HOME}/.backup_envrc"
BACKUP_CRON="/etc/cron.d/${BACKUP_NAME}"
BACKUP_SCRIPT="/usr/local/bin/${BACKUP_NAME}.sh"
...
create_envrc () {
  ...
  echo "Enter the path to the folder you wish to backup:"
...
0 ${BACKUP_HOUR} * * * root ${BACKUP_SCRIPT} ${BACKUP_ENVRC} 2>&1 | /usr/bin/logger -t ${BACKUP_NAME}
EOF
}

install_script () {
  cp ${BACKUP_NAME}.sh "${BACKUP_SCRIPT}"
